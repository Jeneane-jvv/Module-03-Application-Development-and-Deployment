    ...
    switch (role) {
      case 'learner':
        return {
          ...
          message:
            'Start with what the error directly states: which operation failed, where it was observed, and whether the failure describes authentication, connectivity, configuration, validation, or another condition.',
          checks: [
      ...
      case 'recruiter':
        return {
          ...
          message:
            'FirstCommit treats the error as an observed signal, not as an automatic answer. The learner still has to connect that signal to other evidence before deciding on a cause.',
          checks: [
      ...
      case 'educator_assessor':
        return {
          ...
          message:
            'The analyst encourages the learner to record the literal technical signal first, then explain what it may support. This makes the reasoning process easier to assess.',
          checks: [
      ...
      case 'guest':
        return {
          ...
          message:
            'An error can tell you what failed or where a problem became visible, but engineers usually need more evidence before deciding why it happened.',
          checks: [
            'What failed?',
    ...
    switch (role) {
      case 'learner':
        return {
          ...
          message:
            'Inspect the request URL, method, destination, status or network failure, and whether a response was received. Those facts can help distinguish a frontend configuration problem from an API or database problem.',
          checks: [
      ...
      case 'recruiter':
        return {
          ...
          message:
            'The analyst encourages inspection of request destination and response behaviour so the learner can narrow the problem without receiving a preselected diagnosis.',
          checks: [
      ...
      case 'educator_assessor':
        return {
          ...
          message:
            'The learner can use network observations to justify why a failure appears before, at, or after the API boundary rather than making an unsupported statement.',
          checks: [
      ...
      case 'guest':
        return {
          ...
          message:
            'If the browser sends a request to the wrong place, or never receives a response, that can narrow the investigation even before anyone changes code.',
          checks: [
    ...
    switch (role) {
      case 'learner':
        return {
          ...
          message:
            'A configuration value matters when it explains what the running application actually does. Compare the expected production setting with the request, endpoint, or behaviour you observe at runtime.',
          checks: [
      ...
      case 'recruiter':
        return {
          ...
          message:
            'The analyst does not flag a setting simply because it looks suspicious. The learner is encouraged to connect configuration to what the deployed application actually does.',
          checks: [
      ...
      case 'educator_assessor':
        return {
          ...
          message:
            'The learner is prompted to compare expected environment-specific settings with the behaviour of the running application, strengthening the evidence chain.',
          checks: [
      ...
      case 'guest':
        return {
          ...
          message:
            'Engineers compare the configuration that should be active with the behaviour they can observe. A mismatch can become a strong clue.',
          checks: [
    ...
    switch (role) {
      case 'learner':
        return {
          ...
          message:
            'Strong evidence often comes from agreement between different sources: for example a health check, a failed browser request, and a configuration value. Compare what each clue proves and where they overlap.',
          checks: [
      ...
      case 'recruiter':
        return {
          ...
          message:
            'FirstCommit can help surface relationships between technical clues, but the learner remains responsible for deciding what those relationships mean for the final cause assessment.',
          checks: [
      ...
      case 'educator_assessor':
        return {
          ...
          message:
            'The analyst prompts the learner to explain how different evidence sources reinforce or contradict one another instead of presenting an opaque answer.',
          checks: [
      ...
      case 'guest':
        return {
          ...
          message:
            'One clue can be ambiguous. When different observations point in the same direction, the investigation becomes more convincing.',
          checks: [
